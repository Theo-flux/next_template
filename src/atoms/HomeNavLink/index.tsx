...
const HomeNavItem = ({ path, name, className, Icon }: TNavItemProps) => {
  ...
  return (
    <Link href={path}>
      <div
        className={`before:bg-astronautBlue relative w-fit whitespace-nowrap text-lg transition-all duration-150 before:absolute before:bottom-0 before:left-0 before:hidden before:w-full before:rounded-t-lg before:transition-all before:duration-150 before:content-[''] hover:text-black hover:before:h-[5px] lg:py-5 lg:text-xs lg:before:block ${
          isActive
